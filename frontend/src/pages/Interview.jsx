...
export default function Interview() {
  ...
  useEffect(() => {
    ...
    if (jd.includes("senior") || jd.includes("expert") || jd.includes("lead") || jd.includes("staff")) {
      setDifficulty("Senior Level");
    } else if (jd.includes("mid") || jd.includes("intermediate") || jd.includes("experienced")) {
      setDifficulty("Mid Level");
    } else if (jd.includes("entry") || jd.includes("junior") || jd.includes("intern") || jd.includes("fresher") || jd.includes("new grad")) {
  ...
  const handleReset = async () => {
    ...
    setMessages([]); setStarted(false); setCompleted(false);
    setSessionId(null); setSessionConfig(null);
    setResumeFile(null); setResumeText(""); setJobDescription("");
  };
  ...
  const handleSend = async () => {
    ...
    try {
      const allMessages = [
        { role: "system", content: `You are an expert technical interviewer conducting a ${sessionConfig.difficulty} ${sessionConfig.interviewType} interview. Ask one question at a time and give a final evaluation after ${sessionConfig.numQuestions} questions.` },
        ...updated,
      ];
      const res = await sendMessage(sessionId, allMessages, sessionConfig.interviewType, sessionConfig.difficulty, sessionConfig.numQuestions, sessionConfig.resumeText, sessionConfig.jobDescription);
  ...
  const selectClass = "w-full bg-white border border-[#e0d5c8] rounded-xl px-4 py-3 text-sm text-[#1a1007] outline-none focus:border-[#1a1007] focus:ring-2 focus:ring-[#1a1007]/10 transition-all font-medium appearance-none cursor-pointer";
    ...
    <div className="bg-[#fdf8f3]">
      ...
      {!started && (
        <div className="max-w-5xl mx-auto px-6 py-16">
          {/* Page Heading */}
          <div className="mb-12">
            <p className="text-[#c2a882] text-xs font-bold uppercase tracking-widest mb-3">Interview Practice</p>
            <h1 className="text-5xl lg:text-6xl text-[#1a1007] leading-tight mb-4" style={{ fontFamily: "'DM Serif Display', serif" }}>
              Ready to<br /><em className="not-italic text-[#c84b2f]">practice?</em>
            </h1>
            <p className="text-[#8a7060] text-base max-w-md">
              Configure your session below and click Start Interview to begin your AI-powered mock interview.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="space-y-6 bg-white border border-[#e8ddd3] rounded-2xl p-8 shadow-sm">
              <h2 className="text-xs font-bold text-[#8a7060] uppercase tracking-widest border-b border-[#f0e8e0] pb-4">Resume & Job Description</h2>
              ...
              <div>
                <label className="block text-xs font-bold text-[#8a7060] uppercase tracking-widest mb-2">Job Description <span className="text-gray-400 font-normal normal-case">(Optional)</span></label>
                <textarea 
              ...
              <h2 className="text-xs font-bold text-[#8a7060] uppercase tracking-widest border-b border-[#f0e8e0] pb-4 mt-8">Session Settings</h2>

              <div>
                <label className="block text-xs font-bold text-[#8a7060] uppercase tracking-widest mb-2">Interview Type</label>
                <select value={interviewType} onChange={(e) => setInterviewType(e.target.value)} className={selectClass}>
                  {TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#8a7060] uppercase tracking-widest mb-2">Difficulty</label>
              ...
              <div>
                <label className="text-xs font-bold text-[#8a7060] uppercase tracking-widest mb-2 flex justify-between">
                  <span>Questions</span>
                  <span className="text-[#c84b2f] font-bold">{numQuestions}</span>
                </label>
                <input type="range" min="3" max="10" value={numQuestions} onChange={(e) => setNumQuestions(Number(e.target.value))}
                  className="w-full mt-2 cursor-pointer accent-[#c84b2f]" />
                <div className="flex justify-between text-xs text-[#c2b5a6] mt-1"><span>3</span><span>10</span></div>
              </div>
            ...
            <div className="bg-[#1a1007] rounded-2xl p-8 text-[#fdf8f3]">
              <img src="/logo.png" alt="HireReady" className="w-16 h-16 object-contain mb-6 brightness-200 opacity-80" />
              <h3 className="text-2xl mb-3" style={{ fontFamily: "'DM Serif Display', serif" }}>How it works</h3>
              <div className="space-y-4 text-sm text-[#8a7060]">
                {["Configure your type and difficulty above.", "Click Start — the AI interviewer asks one question at a time.", "Answer in the text box and press Send (or Enter).", "After all questions, you get a detailed final evaluation."].map((step, i) => (
                  <div key={i} className="flex gap-3">
                    <span className="text-[#f5a94f] font-bold shrink-0">{i + 1}.</span>
                    <span>{step}</span>
                  </div>
                ))}
              </div>
      ...
      {started && (
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#e8ddd3]">
            <div>
              <p className="text-xs text-[#8a7060] font-bold uppercase tracking-widest mb-1">Active Session</p>
              <h2 className="text-xl font-bold text-[#1a1007]" style={{ fontFamily: "'DM Serif Display', serif" }}>
                {sessionConfig.interviewType} Interview
              </h2>
              <span className="text-xs text-[#c2a882]">{sessionConfig.difficulty} · {sessionConfig.numQuestions} Questions</span>
            </div>
            {!completed && (
              <button onClick={handleReset} className="text-xs font-semibold text-[#c84b2f] hover:text-[#a33a20] bg-[#ffeee9] hover:bg-[#ffddd5] px-4 py-2 rounded-lg border border-[#f9c5b8] transition-all">
                End Session
              </button>
            )}
            {completed && (
              <span className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold px-3 py-1.5 rounded-lg">✅ Complete!</span>
            )}
          </div>
          ...
          <div className="space-y-5 mb-8">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-up`} style={{ animationDelay: `${i * 0.03}s` }}>
                {msg.role === "assistant" && (
                ...
                <div className={`max-w-2xl px-5 py-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                  msg.role === "user"
              ...
              <div className="flex justify-start animate-fade-in">
                ...
                <div className="bg-white border border-[#e8ddd3] px-5 py-4 rounded-2xl rounded-tl-sm shadow-sm">
                  <div className="flex items-center gap-1.5">
                    {[0, 150, 300].map((d) => (
                      <span key={d} className="w-2 h-2 bg-[#c2a882] rounded-full animate-bounce" style={{ animationDelay: `${d}ms` }} />
                    ))}
                  </div>
                </div>
              </div>
          ...
          {completed && (
            <div className="flex gap-4 mt-4">
              <button onClick={handleReset} className="bg-[#1a1007] hover:bg-[#2e1e10] text-[#fdf8f3] font-bold px-6 py-3 rounded-xl text-sm transition-all">
                🔄 New Interview
              </button>
              <button onClick={() => navigate("/history")} className="bg-white hover:bg-[#f0e8e0] text-[#1a1007] font-bold border border-[#e0d5c8] px-6 py-3 rounded-xl text-sm transition-all">
                📚 View History
              </button>
            </div>
          )}
